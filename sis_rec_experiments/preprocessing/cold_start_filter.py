...
class ColdStartFilter:
    def __init__(self, min_user_ratings: int = 20, min_item_ratings: int = 10, 
                 max_user_ratings: int = 500, max_iterations: int = 3):
    ...
    def filter_dataset(self, ratings_df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict]:
        ...
        filtering_report = {
            ...
            "reduction_summary": {
                "ratings_reduction": (original_stats["ratings_count"] - final_stats["ratings_count"]) / original_stats["ratings_count"],
                "users_reduction": (original_stats["users_count"] - final_stats["users_count"]) / original_stats["users_count"],
                "items_reduction": (original_stats["items_count"] - final_stats["items_count"]) / original_stats["items_count"],
    ...
    def _filter_users(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        valid_users = user_counts[
            (user_counts >= self.min_user_ratings) & 
            (user_counts <= self.max_user_ratings)
        ].index
