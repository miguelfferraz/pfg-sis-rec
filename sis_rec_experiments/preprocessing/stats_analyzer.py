...
"""
    ...
    @staticmethod
    def plot_filtering_impact(original_df: pd.DataFrame, filtered_df: pd.DataFrame, 
                            filtering_report: Dict, save_path: str = None):
        ...
        ax3.bar(x - width/2, original_ratings.values, width, label='Original', 
                color='lightblue', alpha=0.7)
        ax3.bar(x + width/2, filtered_ratings.values, width, label='Filtered', 
        ...
        original_values = [original_stats['ratings_count'], 
                         original_stats['users_count'], 
                         original_stats['items_count']]
        final_values = [final_stats['ratings_count'], 
                       final_stats['users_count'], 
                       final_stats['items_count']]
        
        x = np.arange(len(metrics))
        width = 0.35
        
        ax4.bar(x - width/2, original_values, width, label='Original', 
                color='lightblue', alpha=0.7)
        ax4.bar(x + width/2, final_values, width, label='Filtered', 
                color='lightcoral', alpha=0.7)
